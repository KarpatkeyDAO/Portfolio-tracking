...
# getVaultAddress
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def getVaultAddress(blockchain):
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# getCDPRegistryAddress
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def getCDPRegistryAddress(blockchain):
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# getCDPManagerAddress
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def getCDPManagerAddress(blockchain):
...
# getCDPViewerAddress
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def getCDPViewerAddress(blockchain):
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# getUnitCDPViewerData
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def getUnitCDPViewerData(wallet, collateral_address, block, blockchain, **kwargs):
    ...
    try:
        ...
        cdp_registry_address = getCDPRegistryAddress(blockchain)
        cdp_registry_contract = getContract(cdp_registry_address, blockchain, web3 = web3, abi = ABI_CDP_REGISTRY, block = block)
        ...
        if cdp_registry_contract.functions.isAlive(collateral_address, wallet).call(block_identifier = block):
            ...
            cpd_viewer_address = getCDPViewerAddress(blockchain)
            cdp_viewer_contract = getContract(cpd_viewer_address, blockchain, web3 = web3, abi = ABI_CDP_VIEWER, block = block)
            ...
            cdp_manager_address = getCDPManagerAddress(blockchain)
            cdp_manager_contract = getContract(cdp_manager_address, blockchain, web3 = web3, abi = ABI_CDP_MANAGER, block = block)
            ...
            usdp_decimals = getDecimals(usdp_address, blockchain, web3 = web3)
            ...
            cdp_data['utilization_ratio'] = cdp_data['debt_amount'] * 100 / cdp_data['collateral_usd_value']
        ...
        return getUnitCDPViewerData(wallet, collateral_address, block, blockchain, index = index, execution = execution + 1)
        ...
        return getUnitCDPViewerData(wallet, collateral_address, block, blockchain, index = index + 1, execution = execution)


#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
# getUnitCDPData
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def getUnitCDPData(wallet, collateral_address, block, blockchain, **kwargs):
    ...
    try:
        ...
        if cdp_registry_contract.functions.isAlive(collateral_address, wallet).call(block_identifier = block):
            ...
            vault_parameters_contract = getContract(vault_parameters_address, blockchain, web3 = web3, abi = ABI_VAULT_PARAMETERS, block = block)

            collateral_decimals = getDecimals(collateral_address, blockchain, web3 = web3)
            collateral_amount = vault_contract.functions.collaterals(collateral_address, wallet).call(block_identifier = block) / (10**collateral_decimals)

            usdp_address = vault_contract.functions.usdp().call()
            usdp_decimals = getDecimals(usdp_address, blockchain, web3 = web3)
            debt_amount = vault_contract.functions.getTotalDebt(collateral_address, wallet).call(block_identifier = block) / (10**usdp_decimals)
            ...
            vault_manager_parameters_contract = getContract(vault_manager_parameters_address, blockchain, web3 = web3, abi = ABI_VAULT_MANAGER_PARAMETERS, block = block)
            ...
            vault_manager_borrow_fee_parameters_contract = getContract(vault_manager_borrow_fee_parameters_address, blockchain, web3 = web3, abi = ABI_VAULT_MANAGER_BORROW_FEE_PARAMETERS, block = block)
            ...
            # Utilization
            cdp_data['utilization'] = (debt_amount * 100) / (cdp_data['collateral_usd_value'] * cdp_data['icr'] / 100)

            # Debt Limit
            cdp_data['debt_limit'] = vault_parameters_contract.functions.tokenDebtLimit(collateral_address).call(block_identifier = block) / (10**collateral_decimals)
        ...
        return getUnitCDPData(wallet, collateral_address, block, blockchain, index = index, execution = execution + 1)
        ...
        return getUnitCDPData(wallet, collateral_address, block, blockchain, index = index + 1, execution = execution)
...
#---------------------------------------------------------------------------------------------------------------------------------------------------------------------------
def UnitUnderlying(wallet, block, blockchain, **kwargs):
    ...
    try:
        ...
        if len(cdps) == 0:
            ...
            for cdp in cdps:
                ...
                if cdp_registry_contract.functions.isAlive(cdp[0], wallet).call(block_identifier = block):
                    ...
                    collateral_decimals = getDecimals(cdp[0], blockchain, web3 = web3)
        ...
        return UnitUnderlying(wallet, block, blockchain, index = index, execution = execution + 1)
        ...
        return UnitUnderlying(wallet, block, blockchain, index = index + 1, execution = execution)